...
public class RecordProcessor implements IRecordProcessor {
    ...
    private void processSingleRecord(Record record) {
        ...
        try {
            ...
            // The data is either JSON or a URL to compressed JSON
            ...
            LOG.error("Malformed or unavailable data: " + data, e);
        }
    }

    private String readDataFromUrl(URL url) throws IOException {
        // If download fails, retry a couple times with pauses in between.
        final int MAX_RETRIES = 3;

        URLConnection urlConnection = url.openConnection();
        byte[] bytes = null;

        for(int retries=1; bytes == null; ++retries) {
            try (InputStream istrm = urlConnection.getInputStream();
                 ByteArrayOutputStream os = new ByteArrayOutputStream()) {
                byte[] buffer = new byte[0xFFFF];
                for (int len; (len = istrm.read(buffer)) != -1; )
                    os.write(buffer, 0, len);
                os.flush();
                bytes = os.toByteArray();
            } catch (IOException ioe) {
                if (retries < MAX_RETRIES) {
                    LOG.error("Error trying to download large message from " + url.toString() + ".  Retrying...", ioe);
                    try {
                    ...
                    LOG.error("Error trying to download large message from " + url.toString() + ".  Aborting.", ioe);
                    throw ioe;
                }
            }
        }

        String uncompressed = decompress(bytes);
        return uncompressed;
    }
